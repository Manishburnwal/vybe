...
function MessageArea() {
    ...
    const handleSendMessage=async(e)=>{
        ...
        try {
            const result = await axios.post(`${server_url}/api/message/send/${selectedUser._id}`,formData,{withCredentials:true})
    ...
    const getAllMessages=async ()=>{
        try {
            const result = await axios.get(`${server_url}/api/message/getAll/${selectedUser._id}`,{withCredentials:true})
  ...
  return (
    <div className='w-full h-screen bg-black relative'>
      <div className='flex items-center gap-[15px] px-5 py-[10px] fixed top-0 z-[100] bg-black w-full'>
        ...
        <div className='w-[40px] h-[40px] rounded-full overflow-hidden cursor-pointer border-2 border-black' onClick={()=>navigate(`/profile/${selectedUser.userName}`)}>
      ...
      <div className='w-full h-[80px] fixed bottom-0 flex justify-center items-center bg-black z-[100]'>
        <form className='w-[90%] max-w-[800px] h-[80%] rounded-full bg-[#131616] flex items-center gap-[10px] px-5 relative' onSubmit={handleSendMessage}>
            {frontendImage && 
            ...
            <div onClick={()=>imageInput.current.click()}><LuImage className='w-7 h-7 text-white cursor-pointer'/></div>
            {(input || frontendImage) && <button className='w-[60px] h-[40px] rounded-full bg-gradient-to-br from-[#9500ff] to-[#ff0095] flex items-center justify-center cursor-pointer'><IoSend className='w-[23px] h-[23px] text-white'/></button>}
        </form>
      </div>
    </div>
  )
